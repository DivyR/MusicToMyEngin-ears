...
class CRNN2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)  # [batch, 20, 213]  -> [batch, 1, 20, 213]
        x = F.relu(self.conv1(x))  # [batch, 1, 20, 213] -> [batch, 10, 16, 209]
        x = self.maxpool(x)  # [batch, 10, 16, 209] -> [batch, 10, 8, 104]
        x = F.relu(self.conv2(x))  # [batch, 20, 8, 104] -> [batch, 20, 4, 100]
        x = self.maxpool2(x)  # [2, 100]
        x = F.relu(self.conv3(x))  # [40, 1, 98]

        # RNN
        x = x.squeeze(2)  # [batch, 40, 1, 97] -> [batch, 40, 97]
        # each channel is a feature where the x-axis represents time,
        # so we want sequences in the x-direction each with 1 feature from each channel
        x = x.transpose(1, 2)  # [batch, 97, 40]
        out, _ = self.rnn(x)
